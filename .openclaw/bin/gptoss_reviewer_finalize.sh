...
LOG_PATH="$REPO_DIR/docs/gptoss_openclaw_review_log.md"
...
if [[ ! -f "$LOG_PATH" ]]; then
  printf '# GPT-OSS OpenClaw Review Log\n\n' > "$LOG_PATH"
fi
...
commit_output="$($SCRIPT_DIR/gptoss_git_commit.sh reviewer "$commit_message")"
printf '%s\n' "$commit_output"
...
$SCRIPT_DIR/gptoss_task_mark.sh reviewer "$task_id" "$decision" "$commit_sha"
printf 'decision=%s\n' "$decision"
printf 'task=%s\n' "$task_id"
printf 'commit=%s\n' "$commit_sha"
