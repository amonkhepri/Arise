...
QUEUE_PATH = Path("/Users/amunratis/AndroidStudioProjects/Arise-autowork/.openclaw/gptoss_tasks.json")
...
def main(argv):
    if len(argv) < 2:
        raise SystemExit("usage: gptoss_task_queue.py <next|complete-executor|complete-reviewer|list> ...")
    command = argv[1]
    if command == "next":
        if len(argv) != 3:
            raise SystemExit("usage: gptoss_task_queue.py next <executor|reviewer>")
        return cmd_next(argv[2])
    if command == "complete-executor":
        if len(argv) != 4:
            raise SystemExit("usage: gptoss_task_queue.py complete-executor <task-id> <commit-sha>")
        return cmd_complete_executor(argv[2], argv[3])
    if command == "complete-reviewer":
        if len(argv) != 5:
            raise SystemExit("usage: gptoss_task_queue.py complete-reviewer <task-id> <approved|needs_fix> <review-commit-sha>")
