...
QUEUE_PATH="$REPO_DIR/.openclaw/gptoss_tasks.json"
...
commit_output="$($SCRIPT_DIR/gptoss_git_commit.sh executor "$commit_message")"
printf '%s\n' "$commit_output"
...
$SCRIPT_DIR/gptoss_task_mark.sh executor "$task_id" "$commit_sha"
printf 'task=%s\n' "$task_id"
printf 'commit=%s\n' "$commit_sha"
