...
QUEUE_PATH = ROOT / '.openclaw' / 'gptoss_tasks.json'
...
def make_helper_task(task_id, blocked_by, spec):
    task = {
        ...
        'scaffoldCommand': f"./.openclaw/bin/gptoss_seed_invitation_helper.sh {spec['slug']}",
    }
    if blocked_by:
...
def main():
    parser = argparse.ArgumentParser(description='Top up the gpt-oss overnight queue with curated invitation artifact tasks.')
