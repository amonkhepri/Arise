...
def build_goal_record(policy, diagnostic):
    ...
    for key in ("status", "result", "summary", "bugCode", "command", "returncode"):
...
def diagnostic_failure_seed_spec(policy, tasks, diagnostic):
...
def run_completion_supervisor(
    ...
    if diagnostic is not None:
        ...
        seed_spec = diagnostic_failure_seed_spec(policy, tasks, diagnostic)
        if isinstance(seed_spec, str) and seed_spec in {
            "missing_bug_code",
            "unknown_bug_code",
            "goal_not_met_unmapped",
        }:
            save_json(state_path, state)
            return {
                "action": "error",
