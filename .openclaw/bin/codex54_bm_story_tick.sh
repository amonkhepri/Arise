...
case "$action" in
  reviewer)
    run_queue_job "$REVIEWER_ID" reviewer "tick: starting reviewer" "tick: reviewer completed"
    ;;
  executor)
    run_queue_job "$EXECUTOR_ID" executor "tick: starting executor" "tick: executor completed"
    ;;
  done)
    ...
    case "$completion_action" in
      executor)
        run_queue_job \
          "$EXECUTOR_ID" \
          executor \
          "tick: seeded follow-up BU queue; starting executor" \
