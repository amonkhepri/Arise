...
class QueueCompletionSupervisorTest(unittest.TestCase):
    ...
    def test_unmet_goal_without_specific_bug_mapping_seeds_fallback_queue(self):
        ...
        self.assertEqual("messages_not_delivered_end_to_end", seeded["specificSignal"])
        self.assertEqual("bugCode", seeded["specificSignalSource"])
        self.assertTrue(
            any("first briar message" in instruction.lower() for instruction in seeded["instructions"])
        )
        self.assertTrue(
            any("messages_not_delivered_end_to_end" in instruction for instruction in seeded["instructions"])
        )
        ...
        self.assertEqual(["BU-002"], completion["seededTaskIds"])
        self.assertEqual("messages_not_delivered_end_to_end", completion["lastSeedSignal"])
        self.assertEqual("bugCode", completion["lastSeedSignalSource"])

    def test_repeated_specific_signal_does_not_seed_another_followup(self):
        self.write_roadmap("# Briar Invitation-to-Chat Roadmap\n")
        fingerprint = self.module.queue_fingerprint(
            json.loads(self.queue_path.read_text(encoding="utf-8"))["tasks"]
        )
        self.write_state(
            {
                "version": 1,
                "queueCompletion": {
                    "briar_add_user": {
                        "queueFingerprint": "older-fingerprint",
                        "validation": {"status": "passed"},
                        "diagnosticInputs": {"invitation_uri": "briar://real-link"},
                        "diagnosticRunInputs": {"invitation_uri": "briar://real-link"},
                        "lastSeedSignal": "external_invitation_onboarding_did_not_reach_chat",
                        "lastSeedSignalSource": "bugCode",
                    }
                },
            }
        )
            ...
            if "briar-invitation-e2e-diagnose.sh" in command:
                return completed_process(
                    ...
                    stdout=json.dumps(
                        {
                            "result": "bug",
                            "bugCode": "external_invitation_onboarding_did_not_reach_chat",
                            "summary": "External Briar invitation launch did not reach chat and no explicit acceptance failure was logged.",
                        }
                    ),
                )
            return completed_process(command)
        ...
        self.assertEqual("goal_unmet_signal_not_fresh", result["reason"])
        queue = json.loads(self.queue_path.read_text(encoding="utf-8"))
        self.assertEqual(1, len(queue["tasks"]))

        state = json.loads(self.state_path.read_text(encoding="utf-8"))
        completion = state["queueCompletion"]["briar_add_user"]
        self.assertEqual("external_invitation_onboarding_did_not_reach_chat", completion["diagnosticSignal"])
        self.assertEqual("goal_unmet_signal_not_fresh", completion["followupDecision"])
        self.assertEqual("external_invitation_onboarding_did_not_reach_chat", completion["lastSeedSignal"])

        second_result = self.module.run_completion_supervisor(
            queue_path=self.queue_path,
            state_path=self.state_path,
            repo_dir=self.root,
            policy_name="briar_add_user",
            runner=lambda *_: (_ for _ in ()).throw(AssertionError("runner should not be called")),
            roadmap_path=self.roadmap_path,
            invitation_uri="briar://real-link",
        )
        self.assertEqual("hold", second_result["action"])
        self.assertEqual("goal_unmet_signal_not_fresh", second_result["reason"])

    def test_missing_specific_signal_holds_without_seeding_followup(self):
        self.write_roadmap("# Briar Invitation-to-Chat Roadmap\n")
            ...
            if "briar-invitation-e2e-diagnose.sh" in command:
                return completed_process(
                    ...
                    stdout=json.dumps(
                        {
                            "result": "bug",
                            "summary": "Chat still does not behave correctly, but the diagnosis did not classify the failure.",
                        }
                    ),
                )
            return completed_process(command)
        ...
        self.assertEqual("hold", result["action"])
        self.assertEqual("goal_unmet_missing_specific_signal", result["reason"])
        queue = json.loads(self.queue_path.read_text(encoding="utf-8"))
        self.assertEqual(1, len(queue["tasks"]))

        state = json.loads(self.state_path.read_text(encoding="utf-8"))
        completion = state["queueCompletion"]["briar_add_user"]
        self.assertEqual("goal_unmet_missing_specific_signal", completion["followupDecision"])
        self.assertNotIn("lastSeedSignal", completion)
            ...
            def runner(command, cwd):
                if "briar-invitation-e2e-diagnose.sh" in command:
                    ...
                    stdout=json.dumps(
                        {
                            "result": "bug",
                            "failureCode": "chat_opens_but_send_goal_still_unmet",
