...
| Message rendering auth leakage | `app/src/main/java/com/example/rise/item/MessageItem.kt` | Uses `FirebaseAuth.getInstance()` directly for sender-vs-self rendering. | `migrate-now` | Pass current-user identity from UI/viewmodel/model layer; remove static Firebase auth lookup from view item. |
