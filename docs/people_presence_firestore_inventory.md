...
| My account profile | `FirebaseUserRemoteDataSource` (`app/src/main/java/com/example/rise/data/firestore/FirebaseUserRemoteDataSource.kt`) | Fetches/updates the signed-in user profile for account settings. | Router migration PENDING (2025-11-08) | Blocked on introducing `AccountRepository` façade so My Account UI no longer talks to Firestore directly. |
